...
2. Перед запуском сервиса необходимо внести изменения в [config file](configs/config.json) о базе данных и api key
3. Запустить сервис командой `make`

По умолчанию, сервис запустится по адресу `127.0.0.1:8081`. При необходимости адрес так же можно изменить в [config file](configs/config.json).
